...
export default function Store() {
  ...
  //   const PageSize = Math.ceil(allData.length / itemsPerPage)
  ...
  const handleRating = (value) => {
    ...
    const sortRatedFilterArray = ratedFilter.sort(
      (item1, item2) =>
        Math.floor(Number(item2.rating)) - Math.floor(Number(item1.rating))
    );
    ratedFilter.length > 0
      ? setFilterData(sortRatedFilterArray)
  ...
  const card = changeView ? currentPageData?.map((product) => (
                            ...
                            <div className="flex gap-x-2.5">
                              {[...Array(5)].map((_, i) => (
                                <span
                                  ...
                                  className="cursor-pointer p-1 rounded-md shadow border border-gray-100 hover:scale-90 transition-transform ease-in-out"
                                >
                                  {starRating > i ? (
                                    <StarFillIcon className="h-6 w-6 text-yellow-400 hover:text-yellow-500" />
                                  ) : (
                    ...
                    >
                      {({ open }) => (
                        <>
                          ...
                          <Disclosure.Panel className="pt-11">
                            <Slider.Root
                              ...
                              <Slider.Thumb className="shadow-custom relative hover:bg-cyan-500 block w-3.5 h-3.5 bg-gray-50 rounded-full outline-cyan-600">
                                <div className="absolute -mt-8 -ml-[9px]">
                                  <div className="relative shadow-md rounded-full">
                                    ...
                                    <svg
                                      className="absolute text-cyan-400 w-full h-2 left-0 top-100"
                                      x="0px"
                                      y="0px"
                                      viewBox="0 0 255 255"
                                    >
